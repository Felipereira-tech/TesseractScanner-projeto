...
// Criação do provedor de contexto para os gabaritos
export function GabaritosProvider({ children }: { children: React.ReactNode }) {
  ...
  // Estado para armazenar os gabaritos, o status de carregamento e possíveis erros
  ...
  // Efeito para carregar os gabaritos quando o componente for montado
  useEffect(() => {
    recarregar();
  }, [recarregar]);
  ...
  // Fornece o contexto para os componentes filhos, permitindo que eles acessem os gabaritos e as funções para gerenciá-los
  return (
    <GabaritosContext.Provider value={{ gabaritos, loading, erro, addGabarito, updateGabarito, deleteGabarito, recarregar }}>
      {children}
    </GabaritosContext.Provider>
  );
}
